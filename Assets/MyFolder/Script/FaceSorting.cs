...
public class FaceSorting : MonoBehaviour
{
    ...
    private Transform[] tempPos = new Transform[6];
    private Vector3[] startPos = new Vector3[6];

    public static int currentCharaNum;

    private Dictionary<string, RuntimeAnimatorController> _changeAni = new();
    ...
    private void Update()
    {
        ...
        {
            for (int i = 0; i < currentCharaNum; i++)
            {
        ...
        {
            for (int i = 0; i < movingNum; i++)  // 아래에서 위로감
            {
            ...
            for (int i = movingNum+1; i < currentCharaNum; i++) // 위에서 아래로 감
            {
                if (tempPos[i].position != pos[i].position //한번 움직였고
        ...
        {
            Sorting();
            if (inven.check.transform.position.x > 80)
            {
                Inventory.selectFaceName = tempPos[0].name;
            }
        ...
        {
            Inventory.notUsing[i].position = tab15Pos[i].position;
        }
