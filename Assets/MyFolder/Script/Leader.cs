...
namespace MyFolder.Script
{
    public class Leader : MonoBehaviour
    {
        ...
        public Child[] child;
        private List<RbV3> _inputs = new();
        public Leader[] position;
        private int[] _maxDelay = {3,8,13,18,23,24 };
        ...
        readonly float _gravity = -39.24f;
     ...
     */
        ...
        {
            Vector3 result = Vector3.Dot(vec1,normal) / Vector3.Dot(normal, normal) *normal;
        ...
        private void FixedUpdate()
        {
            ...
            if (_currentInput == Mouse) { MouseInput(); } // 클릭 움직임 값 입력
        ...
        { 
            ...
            { 
                ...
                while (_inputs.Count > _maxDelay[5])
                {
        ...
        private void CharMove()
        {
            ...
            {
                Vector3 downVec = TransVec(moveVec, _slopVec);
                Vector3 horizonVec = Vector3.Cross(_planeNormal, _slopVec);
                Vector3 horizonForceVec = TransVec(moveVec, horizonVec);
        ...
        private void HandleWallContact()
        {
            ...
            {
                Vector2 realnormal = TransVec(move, nor);
        ...
        private void SetFacingPos()
        {
            ...
            if (inputCheck)
            {
                lookingPos.localPosition = new Vector3(moveVec.x, 0, moveVec.z).normalized;
            }
        ...
        private void FieldAniControl()
        {
            ...
            Vector3 rightleft = Vector3.Cross(_standardVec, lookingPos.localPosition);
        ...
        {
            float angle = Vector3.Angle(-_standardVec, _needleRot);
            float sign = Vector3.Cross(-_standardVec, _needleRot).y;
            ...
            if (inputCheck)
            {
                needle.rotation = Quaternion.Euler(0, 0, angle * -sign);
            }
        ...
        private void OnCollisionEnter(Collision collision)
        {
            ...
            if (collision.gameObject.CompareTag("Telephote"))
            {
                transform.position = new Vector3(-18f, 38.6f, 16f);
            }
